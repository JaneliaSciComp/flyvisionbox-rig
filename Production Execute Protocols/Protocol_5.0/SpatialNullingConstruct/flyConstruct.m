...
function framePeriodCheck(obj,event,fig)
val=str2double(get(obj,'string'));
if isnan(val) 
    set(obj,'string',num2str(get(obj,'userdata')));
    val=get(obj,'userdata');
else
    FRAME_PERIOD_MAXBITS = 16;
    if val<0||val>2^FRAME_PERIOD_MAXBITS-1
        set(obj,'string',num2str(get(obj,'userdata')));
        val=get(obj,'userdata');
    else
...
for i=1:length(action)
    ...
    switch action(i).command(1)
        ...
        case 3 %video
            vidFrmPeriod = BoxMCU.decodeVideoFramePeriodCmd(action(i).command);
            if vidFrmPeriod==0
                s{end}=['<HTML><FONT color="blue">',s{end},'VIDEOSTOP','</FONT></HTML>'];
            else
                s{end}=['<HTML><FONT color="blue">',s{end},'VIDEO,',...
                        num2str(action(i).command(2)),',',num2str(action(i).command(3)),',',...
                        action(i).userdata,'</FONT></HTML>'];
            end
        case 5 %analog input
...
switch get(gui.ActionType,'value')
    case 1  %Start/Stop Video Acquisition
        ...
        [fpLo fpHi] = BoxMCU.encodeVideoFramePeriod(fp);
        newaction.command(1,1:5)=[3 fpLo fpHi 0 0];
...
ind=1;
for i=1:length(action)
    if action(i).command(1)==3&&action(i).time<newaction.time
        fp = BoxMCU.decodeVideoFramePeriodCmd(action(i).command);
        if fp==0
            %action(i).command is a video STOP command
            PREvid=[];
        else
            %action(i).command is a video START command
            PREvid=i;
        end
    end
...
if ~isempty(PREvid)&&newaction.command(1)==3
    %newaction.command is a video command, and the most immediately
    %preceding video command is a video START command. Convert
    %newaction.command to a video STOP command (if it is not already).
    newaction.command(2)=0;
    newaction.command(3)=0;
    newaction.userdata=[];
end
...
if isnan(N)|N<1
    errordlg('Bad Value of Iterations (N)','Loop Error');
    return
end
k=1;
for i=1:N
...
% AL: Cut+paste shared with ...\Execute\flyExecute.m
function vis=visBuilder(vis,action)
    switch action.command(1)
        ...
        case 3 %Video Acquisition
            ...
            vis.videoRate=[vis.videoRate,BoxMCU.decodeVideoFramePeriodCmd(action.command)];
...
gui.video.sRate=uicontrol(gui.ActionFrame(1),'style','edit','units','normalized',...
    'position',[0.46 0.6 .1 .13],'string','20','fontweight','bold','backgroundcolor','w','userdata',20,'callback',{@framePeriodCheck,gui.fig});
...
gui.Pattern.Direction=uicontrol(gui.ActionFrame(4),'style','popupmenu','units','normalized','horizontalalignment','left',...
    'position',[0.3 0.52 .35 .13],'string',{'Left to Right','Right to Left','Back & Forth','Forth & Back','Sweep Pattern IDs down','Sweep Pattern IDs up','Bounce Pattern IDs down','Bounce Pattern IDs up','Bounce Pattern IDs signed','Sweep Pattern IDs signed','Sweep Pattern IDs signed2'},'fontweight','bold','backgroundcolor','w');
...
%flashpatterns = load('linearcodes64_expanded.txt');
flashpatterns = load('spatialnullingpatterns.txt');
